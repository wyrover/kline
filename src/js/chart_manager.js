...
export class ChartManager {
  ...
  setCaptureMouseWheelDirectly(v) {
    ...
    if (v) $(this._overlayCanvas).bind('mousewheel', Control.mouseWheel)
  ...
  setChartStyle(dsName, style) {
    ...
    switch (style) {
      ...
      case 'OHLC':
        ...
        if (
          dp === undefined ||
          !Util.isInstance(dp, data_providers.MainDataProvider)
        ) {
  ...
  setRunningMode(mode) {
    ...
    if (
      curr_o !== null &&
      curr_o.state !== ctools.CToolObject.state.AfterDraw
    ) {
      pds.delToolObject()
    }
    ...
    if (!drawn) {
      for (let it in this._areas) {
        if (
          ...
          !Util.isInstance(this._areas[it], areas.ChartAreaGroup)
        )
  ...
  clearHighlight() {
    if (
      ...
      this._highlightedFrame !== undefined
    ) {
  ...
  onMouseMove(frameName, x, y, drag) {
    ...
    if (
      ...
      this._capturingMouseArea !== undefined
    ) {
  ...
  deleteToolObject() {
    ...
    if (
      currentObject !== null &&
      currentObject.getState() !== ctools.CToolObject.state.AfterDraw
    ) {
  ...
  setMainIndicator(dsName, indicName) {
    ...
    switch (indicName) {
      case 'MA':
        indic = new indicators.MAIndicator(this._highStockData)
  ...
  setIndicator(areaName, indicName) {
    ...
    if (
      area === null ||
      area === undefined ||
      area.getNameObject().getCompAt(2) === 'main'
    ) {
      return false
    }
    ...
    if (
      dp === null ||
      dp === undefined ||
      !Util.isInstance(dp, data_providers.IndicatorDataProvider)
    ) {
      return false
    }
    ...
    if (Util.isInstance(indic, indicators.VOLUMEIndicator)) {
      ...
      Util.isInstance(indic, indicators.BOLLIndicator) ||
      Util.isInstance(indic, indicators.SARIndicator)
    ) {
  ...
  removeMainIndicator(dsName) {
    ...
    if (
      indicDp === undefined ||
      !Util.isInstance(indicDp, data_providers.IndicatorDataProvider)
    )
      return
