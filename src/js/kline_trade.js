...
export class KlineTrade {
  ...
  pushTrades(array) {
    ...
    for (let i = 0; i < array.length; i++) {
      ...
      if (i >= array.length - this.tradesLimit) {
        ...
        let dateStr =
          this.dateFormatTf(this.tradeDate.getHours()) +
          ':' +
    ...
    let that = this;
    if (this.klineTradeInit) {
      if (this.mytimer !== undefined) clearInterval(this.mytimer);
      this.mytimer = setInterval(() => {
        ...
        this.curPrice = price;
        $('div#price')
          .attr('class', item.type === 'buy' ? 'green' : 'red')
          .text(price);
        j++;
        if (j >= array.length) {
      ...
      if (array.length > 0) {
        ...
        $('div#price')
          .attr(
            'class',
            array[array.length - 1].type === 'buy' ? 'green' : 'red'
          )
          .text(price);
      }
    }
  ...
  asksAndBids(addasks, oldasks, tbDiv) {
      ...
      for (let i = 0; i < addasks.length; i++) {
        if (addasks[i][0] > oldasks[j][0]) {
          ...
          tbDiv
            .find('div:eq(' + j + ')')
            .before(
              "<div class='row add'><span class='price'></span> <span class='amount'>" +
                amounts[0] +
                '<g>' +
                amounts[1] +
                '</g></span></div>'
    ...
    setTimeout(
      ...
      })(tbDiv.find('div.remove'), tbDiv.find('div.add')),
      1000
    );
  }
  ...
  getgview(g) {
    ...
    for (let i = 0; i < g.length; i++) {
      ...
      if (arr.length === 1 || arr[0] !== lastInt) {
        ...
        gstr +=
          "<div class='row'><span class='price'><h>" +
          arr[0] +
          '.</h>' +
          arr[1] +
          "</span> <span class='amount'>" +
