...
export class Control {
  ...
  static clearRefreshCounter() {
    ...
    Control.refreshHandler = setInterval(
  ...
  static parseRequestParam(str) {
    return JSON.parse(
      '{"' + decodeURI(str.replace(/&/g, '","').replace(/=/g, '":"')) + '"}'
    );
  }
  ...
  static requestOverStomp() {
    ...
    if (
      Kline.instance.stompClient &&
      Kline.instance.stompClient.ws.readyState === 1
    ) {
      Kline.instance.stompClient.send(
        Kline.instance.sendPath,
        {},
        JSON.stringify(Control.parseRequestParam(Kline.instance.requestParam))
      );
      return;
    }
    if (Kline.instance.debug) {
  ...
  static requestSuccessHandler(res) {
    ...
    let updateDataRes = Kline.instance.chartMgr.updateData(
      'frame0.k0',
      Kline.instance.data.lines
    );
    Kline.instance.requestParam = Control.setHttpRequestParam(
      Kline.instance.symbol,
      Kline.instance.range,
      null,
      Kline.instance.chartMgr.getDataSource('frame0.k0').getLastDate()
    );

    let intervalTime =
      Kline.instance.intervalTime < Kline.instance.range
        ? Kline.instance.intervalTime
        : Kline.instance.range;
  ...
  static AbortRequest() {
    if (Kline.instance.type !== 'stomp' || !Kline.instance.stompClient) {
      if (
        ...
        Kline.instance.G_HTTP_REQUEST.readyState !== 4
      ) {
  ...
  static chartSwitchLanguage(lang) {
    ...
    $("#chart_language_setting_div li a[name='" + lang + "']").addClass(
      'selected'
    );
  ...
  static onSize(w, h) {
    ...
    let chartWidth = Kline.instance.showTrade
      ? width - Kline.instance.tradeWidth
      : width;
    ...
    let tabBarRect = {};
    tabBarRect.w = toolPanelShown
      ? chartWidth - (toolPanelRect.w + 1)
      : chartWidth;
    ...
    let dropDownSettingsW =
      dropDownSettings.find('.chart_dropdown_t')[0].offsetWidth + 150;
  ...
  static switchTools(name) {
    ...
    if (name === 'on') {
      ...
      if (
        ChartManager.instance._drawingTool === ChartManager.DrawingTool.Cursor
      ) {
        $('#chart_Cursor')
          .parent()
          .addClass('selected');
      } else if (
        ChartManager.instance._drawingTool ===
      ...
      ChartManager.instance.setRunningMode(
        ChartManager.instance._beforeDrawingTool
      );
      ChartManager.instance.redraw('All', true);
    }
  ...
  static switchIndic(name) {
    ...
    if (name === 'on') {
      ...
      else ChartManager.instance.getChart().setIndicator(2, value);
      $('#chart_tabbar')
        .find('a')
        .each(function() {
  ...
  static switchPeriod(name) {
    ...
    ChartManager.instance.setChartStyle(
      'frame0.k0',
      ChartSettings.get().charts.chartStyle
    );
  ...
  static socketConnect() {
    ...
    Kline.instance.stompClient.connect(
      {},
      function() {
        Kline.instance.stompClient.subscribe(
          '/user' + Kline.instance.subscribePath,
          Control.subscribeCallback
        );
        Kline.instance.subscribed = Kline.instance.stompClient.subscribe(
          Kline.instance.subscribePath +
