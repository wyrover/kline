...
export class Plotter extends NamedObject {
  ...
  static drawLines(ctx, points) {
    ...
    for (i = 1; i < cnt; i++) ctx.lineTo(points[i].x, points[i].y)
    if (Plotter.isChrome) {
      ctx.moveTo(points[0].x, points[0].y)
      for (i = 1; i < cnt; i++) ctx.lineTo(points[i].x, points[i].y)
    }
  ...
  static drawDashedLine(ctx, x1, y1, x2, y2, dashLen, dashSolid) {
    if (dashLen < 2) {
      dashLen = 2
    }
    let dX = x2 - x1
    ...
    if (dY === 0) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        ctx.lineTo(x1 + 0.5 + dashX, y1 + 0.5 + dashY)
        x1 += dX
        y1 += dY
      }
      ctx.stroke()
  ...
  static createHorzDashedLine(ctx, x1, x2, y, dashLen, dashSolid) {
    if (dashLen < 2) {
      dashLen = 2
    }
    let dX = x2 - x1
    let count = (dX / dashLen + 0.5) << 0
    for (let i = 0; i < count; i++) {
      ctx.rect(x1, y, dashSolid, 1)
      x1 += dashLen
    }
  }

  static createRectangles(ctx, rects) {
    ctx.beginPath()
    let e,
      i,
      cnt = rects.length
  ...
  static createPolygon(ctx, points) {
    ...
    let i,
      cnt = points.length
    for (i = 1; i < cnt; i++) ctx.lineTo(points[i].x + 0.5, points[i].y + 0.5)
    ctx.closePath()
  }

  static drawString(ctx, str, rect) {
    let w = ctx.measureText(str).width
    if (rect.w < w) {
      return false
    }
    ctx.fillText(str, rect.x, rect.y)
    rect.x += w
    rect.w -= w
    return true
  }
  ...
  static drawPoint(ctx, style, radius, x, y, rotation) {
  ...
  static unclipArea(ctx) {
    ctx.restore()
  }

  static lineTo(ctx, previous, target, flip) {
...
export class CandlestickPlotter extends NamedObject {
  ...
  Draw(ctx) {
    ...
    if (strokePosRects.length > 0) {
      ctx.strokeStyle = theme.getColor(themes.Theme.Color.Positive)
      Plotter.createRectangles(ctx, strokePosRects)
      ctx.stroke()
    }
...
export class CandlestickHLCPlotter extends Plotter {
  ...
  Draw(context) {
    ...
    let ds = mgr.getDataSource(this.getDataSourceName())
    if (
      !Util.isInstance(ds, data_sources.MainDataSource) ||
      ds.getDataCount() < 1
    ) {
      return
    }
  ...
  Draw(context) {
    ...
    for (let i = first; i < last; i++) {
      ...
      if (n < cnt) {
        ...
        if (month === 1) {
          text = year.toString()
          if (lang === 'zh-cn') text += '年'
          else if (lang === 'zh-tw') text += '年'
        } else {
          if (lang === 'zh-cn') text = month.toString() + '月'
          else if (lang === 'zh-tw') text = month.toString() + '月'
          else if (lang === 'en-us') text = TimelinePlotter.MonthConvert[month]
        }
        context.fillStyle = theme.getColor(themes.Theme.Color.Text4)
      }
...
export class TimelineSelectionPlotter extends Plotter {
  ...
  Draw(context) {
    ...
    if (lang === 'zh-cn') {
      ...
      text =
        TimelineSelectionPlotter.MonthConvert[month] +
        ' ' +
        strDate +
        '  ' +
        strHour +
...
export class CToolPlotter extends NamedObject {
  ...
  drawCrossCursor(context) {
    context.strokeStyle = this.theme.getColor(
      themes.Theme.Color.LineColorNormal
    )
  ...
  drawCtrlPt(context) {
    context.strokeStyle = this.theme.getColor(
      themes.Theme.Color.CircleColorStroke
    )
  ...
  highlight(context) {
    context.strokeStyle = this.theme.getColor(
      themes.Theme.Color.LineColorSelected
    )
    this.draw(context)
    this.drawCtrlPt(context)
...
export class DrawStraightLinesPlotter extends CToolPlotter {
  ...
  draw(context) {
    ...
    if (
      ...
      this.crossPt = this.getRectCrossPt(
        this.areaPos,
        this.startPoint,
        this.endPoint
      )
...
export class DrawBiParallelLinesPlotter extends ParallelLinesPlotter {
  ...
  draw(context) {
    ...
    this.getParaPt()
    this.getAreaPos()
    this.crossPt0 = this.getRectCrossPt(
      this.areaPos,
      this.startPoint,
      this.endPoint
    )
    ...
    this.crossPt1 = this.getRectCrossPt(
      this.areaPos,
      this.paraStartPoint,
      this.paraEndPoint
    )
...
export class DrawTriParallelLinesPlotter extends ParallelLinesPlotter {
  ...
  draw(context) {
    ...
    this.crossPt2 = this.getRectCrossPt(
      this.areaPos,
    ...
    for (let i = 0; i < count; i++) {
      ...
      switch (state) {
        case ctools.CToolObject.state.BeforeDraw:
          toolObject.getPlotter().theme = ChartManager.instance.getTheme(
            this.getFrameName()
          )
